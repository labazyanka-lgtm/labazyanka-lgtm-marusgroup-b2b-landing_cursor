...
npm run build && npm run test:e2e # smoke: главная + ответ API без .env
```

### CI

В GitHub Actions (`.github/workflows/ci.yml`) на каждый push и PR выполняются:
`npm ci` → `npm run lint` → `npm run test` → `npm run build` → установка Chromium для Playwright → `npm run test:e2e`.
